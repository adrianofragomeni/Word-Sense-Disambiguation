...
###############################################################################
random.seed('abc')
vocab_fine= {value:key for key,value in utils.load_pickle("../resources/Wordnet_sense_vocab.pickle").items()}
...
###############################################################################     
    ...
    y_fine_grained=tf.placeholder(tf.int32, shape=[None,None], name='fine_grained_label')
    ...
    with tf.variable_scope("dense_finegrained"):
        logits_finegrained = tf.layers.dense(inputs=concat_attention, units=LEN_FINE)
        output_finegrained= tf.identity( logits_finegrained, name= 'finegrained_output')
    ...
    with tf.variable_scope('loss_finegrained'):
        loss_fine=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=output_finegrained,labels=y_fine_grained)
    ...
    return (train, loss, loss_fine, loss_wndomain, loss_lexname,loss_POS, output_finegrained, output_wndomain, output_lexname, input_,input_w_ids,y_fine_grained, y_wndomain, y_lexnames,y_POS, keep_prob)
...
###############################################################################  
    ...
    Y_bn_id=[list(map(int,label.split())) for label in cts.create_dev_labels(path_dev_json,path_dev_labels,type_=0)]
    Y_bn_id=pad_sequences(Y_bn_id, truncating='pre', padding='post', maxlen=MAX_LENGTH)
    ...
    X_candidate_synsets,ids_candidates_synsets=cs.create_candidates(path_dev_json,type_=0)
    ...
    return [X_train,X_train_int, Y_bn_id, Y_wndomain, Y_lex, Y_POS, X_candidate_synsets,X_candidate_wndomain,X_candidate_lexname], [ids_candidates_synsets,ids_candidates_wndomain,ids_candidates_lexname]
...
###############################################################################   
    ...
    Y_bn_id=[list(map(int,label.split())) for label in utils.load_txt("../resources/Train/Wordnet_sense.txt")]
    Y_bn_id=pad_sequences(Y_bn_id, truncating='pre', padding='post', maxlen=MAX_LENGTH)
    ...
    # lists of padded candidate synsets sentences   
    X_candidate_synsets,ids_candidates_synsets=cs.create_candidates(path_train_json,type_=0)
    X_candidate_synsets=pad_sequences(X_candidate_synsets, truncating='pre', padding='post', maxlen=MAX_LENGTH)
...
###############################################################################     
    ...
    train, loss,loss_fine, loss_wndomain, loss_lexname, loss_POS, output_finegrained,output_wndomain, output_lexname, input_,input_w_ids,y_fine_grained, y_wndomain, y_lexnames, y_POS, keep_prob=model()
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(EPOCHS):
            ...
            for _ in range(ITERATIONS): 
                ...
                _, loss_val,loss_val_fine,loss_val_wndomain,loss_val_lexname,loss_val_POS,preds_fine,preds_wndomain,preds_lexname= sess.run([train, loss, loss_fine, loss_wndomain, loss_lexname,loss_POS, output_finegrained,output_wndomain,output_lexname],
                                                                                       feed_dict={input_: batch_x[0],input_w_ids: batch_x[1], y_fine_grained: batch_labels[0],y_wndomain: batch_labels[1], y_lexnames: batch_labels[2], y_POS: batch_labels[3],keep_prob: KEEP_PROB})

                # sum the losses and the accuracies every iteration                
                epoch_loss += loss_val
                epoch_loss_fine += loss_val_fine
                epoch_loss_wndomain  += loss_val_wndomain
                epoch_loss_lexname += loss_val_lexname
                epoch_loss_POS += loss_val_POS

                epoch_acc_fine+=calculate_accuracy(batch_x[1],preds_fine,batch_labels[0],batch_candidates[0],ids_candidates[0])
                epoch_acc_wndomain+=calculate_accuracy(batch_x[1],preds_wndomain,batch_labels[1],batch_candidates[1],ids_candidates[1])
                epoch_acc_lexname+=calculate_accuracy(batch_x[1],preds_lexname,batch_labels[2],batch_candidates[2],ids_candidates[2])
                
            # update the losses and the accuracies every epoch          
            epoch_loss /= ITERATIONS
            epoch_loss_fine /= ITERATIONS
            epoch_loss_wndomain  /= ITERATIONS
            epoch_loss_lexname  /= ITERATIONS
            epoch_loss_POS /= ITERATIONS

            
            epoch_acc_fine /= ITERATIONS
            ...
            for _ in range(DEV_ITERATIONS):
                ...
                loss_val,loss_val_fine,loss_val_wndomain,loss_val_lexname,loss_val_POS, preds_fine,preds_wndomain,preds_lexname= sess.run([loss, loss_fine, loss_wndomain, loss_lexname,loss_POS, output_finegrained,output_wndomain,output_lexname], 
                                                                                                                                          feed_dict={input_: batch_x[0],input_w_ids: batch_x[1], y_fine_grained: batch_labels[0],y_wndomain: batch_labels[1],
                                                                                                                                                     y_lexnames: batch_labels[2],y_POS: batch_labels[3],keep_prob: 1.})

                # sum the losses and the accuracies every iteration           
                dev_loss += loss_val
                dev_loss_fine += loss_val_fine
                dev_loss_wndomain  += loss_val_wndomain
                dev_loss_lexname += loss_val_lexname
                dev_loss_POS += loss_val_POS

                dev_acc_fine+=calculate_accuracy(batch_x[1],preds_fine,batch_labels[0],batch_candidates[0],ids_candidates_dev[0])
                dev_acc_wndomain+=calculate_accuracy(batch_x[1],preds_wndomain,batch_labels[1],batch_candidates[1],ids_candidates_dev[1])
                dev_acc_lexname+=calculate_accuracy(batch_x[1],preds_lexname,batch_labels[2],batch_candidates[2],ids_candidates_dev[2])


            # update the losses and the accuracies every epoch                
            dev_loss/= DEV_ITERATIONS
            dev_loss_fine /= DEV_ITERATIONS
            dev_loss_wndomain  /= DEV_ITERATIONS
            dev_loss_lexname  /= DEV_ITERATIONS
            dev_loss_POS  /= DEV_ITERATIONS

            dev_acc_fine /= DEV_ITERATIONS
        ...
        saver.save(sess, os.path.join('..','resources','model','model_finegrained','model'))

    
    
def batch_creation(X,X_w_id,Y_fine,Y_wndomain,Y_lexname, Y_POS, candidate_synsets,candidate_wndomain,candidate_lexname,batch_size):
###############################################################################
...
###############################################################################
    ...
    while True:
        ...
        for start in range(0, len(X), batch_size):
            ...
            yield ([list(itemgetter(*list(perm[start:end]))(X)),X_w_id[perm[start:end]]],[Y_fine[perm[start:end]], Y_wndomain[perm[start:end]], Y_lexname[perm[start:end]], Y_POS[perm[start:end]]], 
                   [list(itemgetter(*list(perm[start:end]))(candidate_synsets)),list(itemgetter(*list(perm[start:end]))(candidate_wndomain)),list(itemgetter(*list(perm[start:end]))(candidate_lexname)) ])
